...
{
    if (logo_.load(QStringLiteral(":/clatasha/clatasha-logo.png"))) {
    ...
    if (path) {
        const bool loaded = logo_.load(QString::fromUtf8(path));
    ...
    blog(LOG_ERROR, "[Clatasha HUD] Logo failed to load from both resource and package");
}
