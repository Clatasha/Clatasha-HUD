...
void drawSegmentedMeter(QPainter &p, int x, int y, int w, int h, float level)
{
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        p.setPen(Qt::NoPen);
        p.setBrush(on ? QColor(33, 219, 102) : QColor(50, 57, 60, 190));
        p.drawRoundedRect(QRect(x, sy, w, segmentHeight), 1.0, 1.0);
    }
}
} // namespace
...
ClatashaHudWindow::ClatashaHudWindow(QWidget *parent) : QWidget(parent)
{
    ...
    logo_.loadFromData(QByteArray::fromBase64(kLogoBase64), "PNG");
...
QString ClatashaHudWindow::diskSpaceText() const
{
    ...
    if (gib >= 100.0)
        return QStringLiteral("%1 GB").arg(QString::number(gib, 'f', 0));

    return QStringLiteral("%1 GB").arg(QString::number(gib, 'f', 1));
}
...
void ClatashaHudWindow::paintEvent(QPaintEvent *)
{
    ...
    QPainterPath panel;
    panel.addRoundedRect(QRectF(0.5, 0.5, width() - 1.0, height() - 1.0), 6.0, 6.0);
    p.fillPath(panel, QColor(8, 10, 12, 232));
    p.setPen(QPen(QColor(95, 105, 112, 110), 1.0));
    p.drawPath(panel);

    const float desktop = desktopLevel_.load(std::memory_order_relaxed);
    const float mic = micLevel_.load(std::memory_order_relaxed);

    drawSegmentedMeter(p, 8, 15, 4, 49, desktop);
    drawSegmentedMeter(p, 16, 15, 4, 49, mic);

    p.setPen(QColor(230, 28, 46));
    QFont fpsFont(QStringLiteral("Segoe UI"), 29, QFont::Bold);
    fpsFont.setStyleStrategy(QFont::PreferAntialias);
    p.setFont(fpsFont);
    p.drawText(QRect(31, 2, 105, 48), Qt::AlignCenter,
               QString::number(static_cast<int>(std::lround(fps_))));

    p.setPen(QColor(202, 207, 211));
    p.setFont(QFont(QStringLiteral("Segoe UI"), 11, QFont::Normal));
    p.drawText(QRect(60, 43, 91, 22), Qt::AlignCenter, timerText_);

    const bool sessionActive = recordingActive_ || streamingActive_;
    const QPointF spinnerCenter(177.0, 40.0);
    const QRectF spinnerRect(spinnerCenter.x() - 16.0, spinnerCenter.y() - 16.0, 32.0, 32.0);

    if (sessionActive) {
        QPen ringPen(QColor(224, 24, 48), 3.0, Qt::SolidLine, Qt::RoundCap);
        p.setPen(ringPen);
        p.setBrush(Qt::NoBrush);
        p.drawArc(spinnerRect, (90 - spinnerAngle_) * 16, -255 * 16);

        p.setPen(Qt::NoPen);
        p.setBrush(QColor(224, 24, 48));
        p.drawEllipse(spinnerCenter, 2.0, 2.0);
    } else {
        p.setPen(Qt::NoPen);
        p.setBrush(QColor(155, 162, 168));
        p.drawEllipse(QPointF(169.0, 40.0), 2.1, 2.1);
        p.drawEllipse(QPointF(177.0, 40.0), 2.1, 2.1);
        p.drawEllipse(QPointF(185.0, 40.0), 2.1, 2.1);
    }

    p.setPen(QColor(165, 171, 176));
    p.setFont(QFont(QStringLiteral("Segoe UI"), 8, QFont::Normal));
    p.drawText(QRect(153, 62, 52, 14), Qt::AlignCenter, diskText_);

    if (!logo_.isNull()) {
        const QRect logoRect(201, 5, 18, 18);
